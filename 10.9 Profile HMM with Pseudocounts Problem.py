inPath = 'D:/OneDrive - BYU/BIO 364/Test Data/10.9input.txt'
with open(inPath) as input:
    inputArray = input.read().split('--------')
threshold = float(inputArray[0].split()[0])
pseudocount = float(inputArray[0].split()[1])
    ...
    if total > 0:
        for i in range(len(transmission[node])):
            if transmission[node][i] != 0:
                transmission[node][i] = transmission[node][i]/total
for i in range(2):
    for j in range(3):
        transmission[nodes[i]][j+1] += pseudocount
        bottom = len(nodes) - 4 + j
        right = len(nodes) - 2 + i
        transmission[nodes[bottom]][right] += pseudocount
for i in range(len(nodes) - 6):
    for j in range(len(nodes) - 6):
        transmission[nodes[i+2]][j+4] += pseudocount
for node in transmission:
    total = 0
    for i in range(len(transmission[node])):
        total += transmission[node][i]
    ...
    if total > 0:
        for i in range(len(emission[node])):
            if emission[node][i] != 0:
                emission[node][i] = emission[node][i]/total
for node in emission:
    total = 0
    for i in range(len(emission[node])):
        total += emission[node][i]
    if total > 0 or 'I' in node:
        for i in range(len(emission[node])):
            emission[node][i] += pseudocount
for node in emission:
    total = 0
    for i in range(len(emission[node])):
        total += emission[node][i]
